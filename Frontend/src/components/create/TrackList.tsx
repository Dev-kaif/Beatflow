...
import setPublishedStatus, { deleteSong, renameSong } from "@/actions/song";
...
export function TrackList({ tracks }: { tracks: Track[] }) {
  ...
  useEffect(() => {
    if (track == null) {
      setIsplayTrackId(null);
    } else {
      setIsplayTrackId(track.id);
    }
  }, [track]);
    ...
    <div className="flex flex-1 flex-col overflow-y-scroll">
                ...
                default:
                  return (
                    <div
                      ...
                      onClick={() =>
                        isplayTrackId !== track.id && handleTrackSelect(track)
                      }
                    >
                      {/* Thumbnail */}
      ...
      {trackToDelete && (
        <DeleteDialog
          ...
          onDelete={(trackId) => deleteSong(trackId)}
        />
      )}
    </div>
  );
